...
@bot.slash_command(name="kazuate", description="数当てゲーム", guild_ids=[DISCORD_SERVER_IDS])
async def on_message(
    message: discord.Message,
    kazu: Option(int, required=False, description="難易度を変えれるよ")
):
    ...
    for i in range(1,max_count+1):
        ...
        async with message.channel.typing():
            reply = await bot.wait_for("message", check=kazuatecheck)
